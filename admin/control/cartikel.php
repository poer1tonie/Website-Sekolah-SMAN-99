<?php 	 
include('function/fartikel.php'); 
$fartikel = new fartikel();   
...
{ 
  $status=$_GET['yaksi']; 
	...
	{ 
      ...
      $dataku = $fartikel->tampil_data();
      include "./view/vartikel.php";   
	}

  if($status=='2') 
  {  
      include "./view/vartikel_add.php";   
  }
  ...
  if($status=='3')   // simpan tambah data slide
              ...
              // Validasi tipe file yang diperbolehkan
                  ...
                  if (!in_array($imageFileType, $allowed_file_types)) {
                      ...
                      if (move_uploaded_file($temp_name, $target_file)) {
                          ...
                          $result = $fartikel->input_data($category, $judul,$isi_artikel, $newFileName);
                          ...
                          echo '<meta http-equiv="refresh" content="1;url=?aksi=Master&xaksi=artikel&yaksi=1">';
                      } else {
     ...
     if($status=='4') 
        ...
        { 
            $id=$_GET['id'];
            $dataku = $fartikel->cari_data($id);
            include "./view/vartikel_ubah.php";   
        }
          ...
          {   
            ...
            if (isset($_POST['update'])) {
                ...
                if ($_FILES['gambar']['name']) {
                    $target_dir = $_SERVER['DOCUMENT_ROOT'] . "/profile/images/artikel/";
                    ...
                    if (!in_array($imageFileType, $allowed_file_types)) {
                        ...
                        if (move_uploaded_file($temp_name, $target_file)) {
                            ...
                            $result = $fartikel->ubah_data($id, $category, $judul, $isi_artikel, $newFileName);
                    ...
                    $result = $fartikel->ubah_data($id, $category, $judul, $isi_artikel, $gambar_lama);
                    if ($result) {
                        echo '<div class="alert alert-success">Data berhasil diubah.</div>';
                    } else {
                        echo '<div class="alert alert-danger">Data gagal diubah.</div>';
